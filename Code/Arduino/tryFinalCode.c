...
float pid_i_gain_roll = 20;
float pid_d_gain_roll = 200;
int   pid_max_roll    = 4000;
...
void loop(void)
{
  ...
  switch(En_StateMachineGlobal)
  {
    ...
    { 
		AutomaticDroneTakeOff();
    }
	break;
	
	case State_AutomaticDroneLanding:
	{
		AutomaticDroneLanding();
	}
    break;
  }
...
void GetBluetoothCommand(void)
{
  ...
  if( U8_BluetoothMessage == BLT_ZERO )
  {
	...
	Serial.println("Temperature Measurement State");
  }
  ...
  else if( U8_BluetoothMessage == BLT_SEVEN)
  {
	  En_StateMachineGlobal = State_AutomaticDroneTakeOff;
	  Serial.println("Automatic Take off State");
  }
  else if( U8_BluetoothMessage == BLT_EIGHT)
  {
	  En_StateMachineGlobal = State_AutomaticDroneLanding;
	  Serial.println("Automatic Landing State");
  }
}
...
void ManualDroneControl(void)
{
  ...
  U8_BluetoothMessage = Serial.read();
	...
	if(U8_BluetoothMessage == BLT_ONE)
	{
		throttle++;
		Serial.println(throttle);
	}
	else if(U8_BluetoothMessage == BLT_ZERO)
	{
		throttle--;
		Serial.println(throttle);
	}
	else if(U8_BluetoothMessage == BLT_NINE)
	{
		Serial.println("Power Saving State");
		En_StateMachineGlobal = State_PowerSaving;
	}
	else if(U8_BluetoothMessage == BLT_FOUR)	/* lean front left */
	{
		u8 U8_tempfl = 0;
		
		U8_tempfl = (fl - 100);
		Serial.println("LEANING FRONT LEFT");		
		FrontLeft.writeMicroseconds(U8_tempfl);
		delay(200);		
	}
	else if(U8_BluetoothMessage == BLT_FIVE)	/* lean front right */
	{
		u8 U8_tempfr = 0;

		U8_tempfr = (fr - 100);
		Serial.println("LEANING FRONT RIHT");		
		FrontLeft.writeMicroseconds(U8_tempfr);
		delay(200);	

		for(int i=0;i<10;i++)
		{
			Serial.println(U8_tempfr);
		}
	}
	else if(U8_BluetoothMessage == BLT_SIX)	/* lean rear left */
	{
		u8 U8_temprl = 0;

		U8_temprl = (rl - 100);
		Serial.println("LEANING REAR LEFT");		
		FrontLeft.writeMicroseconds(U8_temprl);
		delay(200);		
	}
	else if(U8_BluetoothMessage == BLT_TWO)	/* lean rear right */
	{
		u8 U8_temprr = 0;

		U8_temprr = (rr - 100);
		Serial.println("LEANING REAR RIGHT");		
		FrontLeft.writeMicroseconds(U8_temprr);
		delay(200);		
	}
	calculatePid();
...
void MeasureTemperature(void)
{
	...
	if(U8_BluetoothMessage == BLT_NINE)
	{
		Serial.println("Power Saving State");
		En_StateMachineGlobal = State_PowerSaving;
	}
...
void AutomaticDroneTakeOff(void)
{
	u8 U8_BluetoothMessage; 
	U8_BluetoothMessage = Serial.read();
...
void AutomaticDroneLanding(void)
{
	if( throttle > 1100)
	{
		throttle--;
...
void GyroInit(void)
{
  ...
  // wait for ready
    ...
    mpu.setXGyroOffset(135);  //220
    mpu.setYGyroOffset(43);   //76
    mpu.setZGyroOffset(34);    // -85
    mpu.setZAccelOffset(1250); // 1788//1688 factory default for my test chip
